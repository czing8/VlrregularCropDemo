...
- (void)addPointsAt:(NSArray *)points
{
    ...
    {
        UIView *pointView = [self getPointView:i at:point.CGPointValue];
        [tmp addObject:pointView];
...
- (UIImage *)deleteBackgroundOfImage:(UIImageView *)image
{
    ...
    
    {
        ...
        CGPoint p1 = [VCropperView convertCGPoint:[[points objectAtIndex:0] CGPointValue] fromRect1:image.frame.size toRect2:image.image.size];
        [aPath moveToPoint:CGPointMake(p1.x, p1.y)];
        
        for (uint i=1; i<points.count; i++)
        {
            CGPoint p = [VCropperView convertCGPoint:[[points objectAtIndex:i] CGPointValue] fromRect1:image.frame.size toRect2:image.image.size];
